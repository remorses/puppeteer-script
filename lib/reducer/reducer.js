...
const anticaptcha_1 = require("./anticaptcha/anticaptcha");
...
exports.reducer = async (state, action) => {
    const page = await state;
    switch (action.method) {
        case "go-to": {
            const url = action.arg;
            await helpers_1.preparePage(page);
            await page.goto(url);
            return page;
        }
        ...
        case "click": {
            ...
            // logger(selector)
            const element = await helpers_1.findElement(page, selector, containing);
            if (!element)
                throw new Error("no element found contining " + containing);
            await helpers_1.waitForElement(element);
        ...
        case "new-page": {
            ...
            await helpers_1.preparePage(_page);
            return _page;
        }
        case "wait": {
            const time = action.arg;
            time ? await page.waitFor(time) : await helpers_1.waitForLoad(page);
            return page;
        }
        case "echo": {
        ...
        case "target-page": {
            ...
            if (index < 0) {
                ...
                await helpers_1.preparePage(pages[index]);
                return pages[index];
            }
        }
        case "screenshot": {
            const file = action.arg;
            let path = path_1.join(WORKING_DIR, file);
            page.screenshot({ path });
            return page;
        }
        case "inject": {
            const path = action.arg;
            const file = fs.readFileSync(path_1.join(WORKING_DIR, path), 'utf8');
            page.evaluate(file);
            return page;
        }
        case "evaluate": {
        ...
        case "set-cookies": {
            const path = action.arg;
            const file = require(path_1.join(WORKING_DIR, path));
        ...
        case "export-html": {
            ...
            await fs.writeFile(path_1.join(WORKING_DIR, path), content);
            return page;
        }
        case "solve-nocaptcha": {
            ...
            const siteKey = await helpers_1.getAttribute(page, elem, "site-key");
            if (!process.env["ANTICAPTCHA_KEY"])
                throw Error("please put ANTICAPTCHA_KEY in environment or anticaptcha-key in script file");
            const solution = await anticaptcha_1.solveNoCaptcha(page, process.env["ANTICAPTCHA_KEY"], siteKey, process.env["ANTICAPTCHA_CALLBACK"]);
